...
OUT_META    = PACKAGE_ROOT / 'data' / 'outputs' / 'meta_1476_reg.pkl'
OUT_BASE    = PACKAGE_ROOT / 'data' / 'outputs' / 'full_year_baseline_reg.pkl'

print("=" * 75, flush=True)
print("[Step 2] Building (1476, 70, 100) Dataset with 18-Class Physics Baseline (Regularity)", flush=True)
print("=" * 75, flush=True)
...
print(f"3/4 正在映射 1,476 個目的地通道並計算規律度 (Regularity Score)...", flush=True)
active_routes_info = []
for pair_key, raw_arr in od_ts.items():
    ...
    # 計算每一週之間的相關係數 (Consecutive Weekly Correlation)，僅使用震前 (一月以前) 的資料
    pre_jan_dates = [d for d in dates_str if d < '20240101']
    history_pre = []
    for d in pre_jan_dates:
        oi = obs_date_to_idx.get(d)
        ci = cal_date_to_idx.get(d)
        if oi is not None and ci is not None:
            v_val = raw_arr[oi]
            b_val = b_366[ci]
            if not np.isnan(v_val):
                res_val = float(v_val) - float(b_val)
                history_pre.append((datetime.strptime(d, '%Y%m%d'), res_val))
                
    weeks = {}
    for dt, val in history_pre:
        wy = dt.isocalendar()[:2]
        if wy not in weeks:
            weeks[wy] = {}
        weeks[wy][dt.weekday()] = val
        
    full_weeks = []
    for wy, wdata in sorted(weeks.items()):
        if len(wdata) == 7:
            full_weeks.append(np.array([wdata[i] for i in range(7)]))
            
    corrs = []
    for i in range(len(full_weeks)-1):
        v1 = full_weeks[i]
        v2 = full_weeks[i+1]
        std1 = np.std(v1)
        std2 = np.std(v2)
    ...
    is_regular = bool(correlation > 0.4)
    reg_label = " (Regular)" if is_regular else " (Irregular)"
    
    old_cls_name = route_class_info[pair_key]['class_name']
    new_cls_name = old_cls_name + reg_label
    route_class_info[pair_key]['class_name'] = new_cls_name
    route_class_info[pair_key]['is_regular'] = is_regular
    route_class_info[pair_key]['correlation'] = float(correlation)
    
    class_stats[new_cls_name] = class_stats.get(new_cls_name, 0) + 1
    ...
    active_routes_info.append({
        ...
        'mean_v': mean_v,
        'max_v': max_v,
        'class_name': new_cls_name,
        'class_id': route_class_info[pair_key]['class_id'],
        'is_regular': is_regular,
        'correlation': float(correlation)
    })

print("✅ 18 大類別 (規律/不規律) 分佈統計:")
for k, v in sorted(class_stats.items(), key=lambda x: -x[1]):
    print(f"  • {k:<60}: {v:>5} 條", flush=True)
...
print(f"✅ 18 大類別物理 Baseline、4D 條件特徵與元數據儲存完成: {OUT_META}", flush=True)
print("=" * 75, flush=True)
