...
PRED_TSV_MIX  = PACKAGE_ROOT / 'data' / 'outputs' / 'dest1476_predictions.tsv'
PRED_TSV_PURE = PACKAGE_ROOT / 'data' / 'outputs' / 'dest1476_predictions_pure.tsv'
...
pred_mix_data = parse_pred_tsv(PRED_TSV_MIX)
pred_pure_data = parse_pred_tsv(PRED_TSV_PURE)
...
with tab1:
    st.sidebar.markdown("### 🎛️ 預測模型版本切換 (Model Switcher)")
    model_view_option = st.sidebar.radio(
        "選擇預測曲線顯示版本:",
        [
            "✨ 非 Pure 混和版 (ψ7 通勤 0.6 + Diff 0.4)",
            "🌌 Pure 純擴散版 (純 Diffusion 1.0, 無 ψ7)",
            "📊 同步對比模式 (同一圖表同時顯示兩者)"
        ],
        index=0
    )
    ...
    if selected_pair:
        ...
        pred_mix_vals = [np.nan] * 366
        for d_str_k, od_map in pred_mix_data.items():
            if d_str_k in cal_date_to_idx:
                val = od_map.get(o_str, {}).get(d_str, np.nan)
                pred_mix_vals[cal_date_to_idx[d_str_k]] = val

        pred_pure_vals = [np.nan] * 366
        for d_str_k, od_map in pred_pure_data.items():
            if d_str_k in cal_date_to_idx:
                val = od_map.get(o_str, {}).get(d_str, np.nan)
                pred_pure_vals[cal_date_to_idx[d_str_k]] = val

        df_ts = pd.DataFrame({
            "Date": [datetime.strptime(d, "%Y%m%d") for d in cal_dates],
            "Actual": y_obs,
            "Baseline": b_vals,
            "Pred_Mix": pred_mix_vals,
            "Pred_Pure": pred_pure_vals
        })
        ...
        fig.add_trace(go.Scatter(
            ...
            mode="lines", name="⚪ 9-Class 物理 Baseline (強指數/樣條轉移)",
            line=dict(color="#94A3B8", width=2.5, dash="dash")
        ))

        # 4. 依據使用者選擇繪製預測曲線
        if "非 Pure" in model_view_option or "對比模式" in model_view_option:
            fig.add_trace(go.Scatter(
                x=df_ts["Date"], y=df_ts["Pred_Mix"],
                mode="lines", name="🟢 混和版預測 (ψ7週期 0.6 + Diff 0.4)",
                line=dict(color="#38BDF8", width=2.5)
            ))

        if "Pure 純擴散" in model_view_option or "對比模式" in model_view_option:
            fig.add_trace(go.Scatter(
                x=df_ts["Date"], y=df_ts["Pred_Pure"],
                mode="lines", name="🟣 Pure 純擴散 (純 Diffusion 1.0, 無ψ7)",
                line=dict(color="#C084FC", width=2.2, dash="dot" if "對比模式" in model_view_option else "solid")
            ))
