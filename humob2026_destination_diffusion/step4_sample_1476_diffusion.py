...
# 🌟 不再做全域 Clip，因為會導致極端值全部被壓在天花板變成方波
# 取而代之，我們會在個別路線做標準化 Z-Score
...
for r in meta_1476['active_routes']:
    ...
    # 🌟 2. 釋放人工天花板：直接使用真實的 sigma_i 與歷史週末波動！
    if mean_v >= 1.0:
        psi_scale = 1.0
        diff_scale = sig_i * 0.7 # 留 30% 的變異數空間給 psi_comp
    else:
        # 極稀疏路線稍微壓抑，避免雜訊過大
        psi_scale = 0.5
        diff_scale = sig_i * 0.5
        
    if is_diag:
        diff_scale = sig_i * 0.8
    ...
    if len(pre_obs) >= 14 and mean_v >= 1.0:
        ...
        for j, ci in enumerate(blind_idxs):
            tau = j / float(N_BLIND)
            if cls_id in (5, 6): gate = 1.0
            elif cls_id == 4: gate = 0.5 + 0.5 * tau
            elif cls_id in (2, 3, 7, 8): gate = 0.3 + 0.4 * tau 
            else: gate = 0.6
